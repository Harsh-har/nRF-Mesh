...
public class GenericLevelSet extends ApplicationMessage {
    ...
    void assembleMessageParameters() {
        ...
        if (mIsLongCommand) {
            ...
            final ByteBuffer paramsBuffer = ByteBuffer.allocate(REGULAR_PARAMS_LENGTH)
                    .order(ByteOrder.LITTLE_ENDIAN);
    ...
    public static class Builder {
        ...
        public GenericLevelSet build() {
            if (isLongCommand) {
                return new GenericLevelSet(appKey, command, length, dataArray, tid);
            } else {
                return new GenericLevelSet(appKey, command, level, tid);
            }
        }
    }
}